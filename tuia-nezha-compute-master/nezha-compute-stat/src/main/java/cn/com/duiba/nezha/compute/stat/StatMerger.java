...
public class StatMerger {
    ...
    public static StatDto dimsMerge(StatDto appDto,
        ...
        if (globalDto != null && globalDto.getLaunchCnt() > 50) {
            ret = globalDto;
            isNewAdvert = isNewAdvert(globalDto.getLaunchCnt());
        }
    ...
    public static Boolean isNewAdvert(Long launchCnt) {
        if (launchCnt != null && launchCnt > 500) {
            return false;
        } else {
            return true;
        }
    ...
    public static StatDto intervalMerge(
        ...
        StatDto hourMergeDto = intervalMerge(statDtoHourMap, 6, 1L);
        StatDto dayMergeDto = intervalMerge(statDtoDayMap, 4, 2L);
...
//        System.out.println("dayMergeDto=" + JSON.toJSONString(dayMergeDto));
        ...
        double historyCtr = MergerUtil.getCtrWithBias(dayChargeCnt, dayLaunchCnt, 100, 0.3, 100, 5);
        double historyCvr = MergerUtil.getCtrWithBias(dayActClickCnt, dayChargeCnt, 100, 0.05, 100, 5);

        double mergeCtr = MergerUtil.getCtrWithBias(hourChargeCnt, hourLaunchCnt, 100, historyCtr, 100, 5);
        double mergeCvr = MergerUtil.getCtrWithBias(hourActClickCnt, hourChargeCnt, 100, historyCvr, 100, 5);
    ...
    private static StatDto intervalMerge(Map<Long, StatDto> statDtoMap, int upLimit, Long type) throws Exception {
        ...
        Map<String, Long> backendMap = new HashMap<>();
                    ...
                    if (AssertUtil.isNotEmpty(backendMapTmp)) {
                        for (Map.Entry<String, Long> entry1 : backendMapTmp.entrySet()) {
                            ...
                            if (backendMap.containsKey(subType)) {
                                backendMap.put(subType, MathUtil.add(MathUtil.dot(beCnt, weight), backendMap.get(subType)));
                            } else {
                                backendMap.put(subType, beCnt);
                            }
                        }
                    }
                    i++;
        ...
        if (i > 0) {
            ...
            for (Map.Entry<String, Long> entry3 : backendMap.entrySet()) {
                backendMap.put(entry3.getKey(), MathUtil.division(entry3.getValue(), weightCnt / i));
            }
