    ...
    public boolean createTableBySplitKeys(String tableName, List<String> columnFamily, String[] sKeys) throws Exception {
        try {
            if (StringUtils.isBlank(tableName) || columnFamily == null
                    || columnFamily.size() < 0 || sKeys == null || sKeys.length < 0) {
            }
            if (admin.tableExists(TableName.valueOf(tableName))) {
                ...
                byte[][] splitKeys = getSplitKeys(sKeys);
        ...
        try {
            if (quailifer != null &&
                ...
                for (int i = 0; i < quailifer.length; i++) {
                    p.addColumn(family.getBytes(), Bytes.toBytes(quailifer[i]), Bytes.toBytes(value[i]));
                }
                getHTable(tableName).put(p);
            }
        } catch (IOException e) {
    ...
    public void insertColumeValues(String tableName, String rowKey, String family, String[] quailifer, long value[]) {
        try {
            if (quailifer != null &&
                ...
                Put p = new Put(Bytes.toBytes(rowKey));
                for (int i = 0; i < quailifer.length; i++) {
                    p.addColumn(family.getBytes(), quailifer[i].getBytes(), Bytes.toBytes(value[i]));
                }
                getHTable(tableName).put(p);
            }
        } catch (IOException e) {
    ...
    public void incrementColumeValues(String tableName, String rowKey, String family, String[] quailifer, Long value[]) {
        try {
            if (quailifer != null &&
                ...
                for (int i = 0; i < quailifer.length; i++) {
                    if (value[i] != null) {
                        increment.addColumn(family.getBytes(), quailifer[i].getBytes(), value[i]);
                    }
    ...
    public void incrementColumeValues(String tableName, String rowKey, String family, int[] quailifer, long value[]) {
        try {
            if (quailifer != null &&
                    value != null &&
                    family != null &&
                    quailifer.length == value.length && quailifer.length > 0) {
                Increment increment = new Increment(rowKey.getBytes());

                for (int i = 0; i < quailifer.length; i++) {

                    increment.addColumn(family.getBytes(), Bytes.toBytes(quailifer[i] + ""), value[i]);
//                    System.out.println("rowk=" + rowKey + ",fam=" + family + ",col=" + quailifer[i] + "" + ",val=" + value[i]);
                }
                getHTable(tableName).increment(increment);
            }
        } catch (IOException e) {
    ...
    public void getRows(String tableName, List<String> rowKeyList, String familyName, Set<String> cSet, QueryCallback callback) throws Exception {
        ...
        List<Result> list = new ArrayList<Result>();
        try {
            if (AssertUtil.isAllNotEmpty(rowKeyList, cSet)) {
                ...
                for (String rowKey : rowKeyList) {
                    ...
                    if (getList.size() > 1000) {
                        ...
                        getHTable(tableName).batch(batch, results);

                        list.addAll(toResult(results));
                        getList.clear();
                        batch.clear();
                    }
                }
    ...
    public void deleteRow(String tableName, String row) {
        ...
        if (row != null) {
            Delete delete = new Delete(row.getBytes());
            try {
                getHTable(tableName).delete(delete);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
