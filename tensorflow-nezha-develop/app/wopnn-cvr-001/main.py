...
def model_fn(features, labels, mode, params):
    ...
    first_order_weight = tf.get_variable(name='first_order_weight', shape=[layers[0], field_size, embedding_size + 1],
                                         initializer=tf.contrib.layers.xavier_initializer(), trainable=True)
    linear_bias = tf.get_variable(name='first_order_bias', shape=[layers[0]],
    ...
    outer_second_weight = tf.get_variable(name='second_order_weight', shape=[layers[0], embedding_size+1, embedding_size+1],
                                          initializer=tf.contrib.layers.xavier_initializer(), trainable=True)
    ...
    with tf.variable_scope("Deep-part"):
        ...
        # first_inputs = tf.reshape(feat_vals, shape=[-1, field_size * (embedding_size + 1) + 1])
        ...
        for i in range(layers[0]):
            linear_output.append(tf.reshape(
                tf.reduce_sum(tf.multiply(embeddings, first_order_weight[i]), axis=[1, 2]),
                shape=(-1, 1)))  # N * 1
        ...
        for i in range(layers[0]):
            theta = tf.multiply(p, tf.expand_dims(outer_second_weight[i], 0))  # N * K * K
        ...
        deep_inputs = tf.nn.relu(tf.add(tf.add(lz, lp), linear_bias))
        ...
        for i in range(len(layers[1:])):
            ...
            deep_inputs = tf.contrib.layers.fully_connected(inputs=deep_inputs, num_outputs=layers[i + 1], \
                                                            # normalizer_fn=normalizer_fn, normalizer_params=normalizer_params, \
                                                            weights_regularizer=tf.contrib.layers.l2_regularizer(
                                                                l2_reg), scope='mlp%d' % (i + 1))
            if batch_norm:
                deep_inputs = batch_norm_layer(deep_inputs, train_phase=train_phase, scope_bn='bn_%d' % (i + 1),
            ...
            if mode == tf.estimator.ModeKeys.TRAIN:
                deep_inputs = tf.nn.dropout(deep_inputs, keep_prob=dropout[
                    i + 1])  # Apply Dropout after all BN layers and set dropout=0.8(drop_ratio=0.2)
...
def main(_):
    ...
    epochs_ = 5
    batch_size_ = 64  # FLAGS.batch_size
