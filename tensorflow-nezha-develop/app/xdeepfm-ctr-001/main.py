...
def _build_linear(params, feat_vals):
    field_size = params["field_size"]
    embedding_size = params["embedding_size"]
    w_linear = tf.get_variable(name='w',
                               shape=[field_size * (embedding_size + 1) + 1, 1],
                               dtype=tf.float32,
    ...
    with tf.variable_scope("linear_part") as scope:
        ...
        x = tf.reshape(feat_vals, shape=[-1, field_size * (embedding_size + 1) + 1])
        linear_output = tf.add(tf.matmul(x, w_linear), b_linear)
        #         linear_output = tf.nn.xw_plus_b(x, w_linear, b_linear)
...
def _build_extreme_FM(params, feat_vals, res=False, direct=False, bias=False, reduce_D=False, f_dim=2):
    field_size = params["field_size"]
    embedding_size = params["embedding_size"]
    ...
    nn_input = tf.reshape(fm_m_v, shape=[-1, field_size, embedding_size + 1])
    ...
    with tf.variable_scope("exfm_part") as scope:
        ...
        if res:
            ...
            #                                activation="relu",
            #                                layer_idx=0)
            exFM_out1 = tf.nn.relu(exFM_out0)
...
def _build_dnn(params, feat_vals, embed_layer_size):
    field_size = params["field_size"]
    embedding_size = params["embedding_size"]
    ...
    layer_sizes = params["layers_sizes"]

    w_fm_nn_input = tf.reshape(feat_vals, shape=[-1, field_size * (embedding_size + 1) + 1])
    last_layer_size = embed_layer_size
    layer_idx = 0
    hidden_nn_layers = []
    hidden_nn_layers.append(w_fm_nn_input)
    with tf.variable_scope("nn_part") as scope:
        for idx, layer_size in enumerate(layer_sizes):
            ...
            curr_hidden_nn_layer = tf.nn.relu(curr_hidden_nn_layer)
            curr_hidden_nn_layer = tf.nn.dropout(curr_hidden_nn_layer, keep_prob=dropout[layer_idx])
...
def model_fn(features, labels, mode, params):
    ...
    # ------bulid weights------
    xde_B = tf.get_variable(name='concat_bias', shape=[1], initializer=tf.constant_initializer(0.0))

    linear_output = _build_linear(params, feat_vals)
    exFM_out = _build_extreme_FM(params, feat_vals, res=False, direct=False, bias=False, reduce_D=False, f_dim=2)
    nn_output = _build_dnn(params, feat_vals, field_size * (embedding_size + 1) + 1)
    print "shape:", linear_output.shape, exFM_out.shape, nn_output.shape

    with tf.variable_scope("xdeepfm-out"):
        y_bias = xde_B * tf.ones_like(fm_bias, dtype=tf.float32)  # None * 1
        #         y = tf.add(linear_output,exFM_out,nn_output)
        y = tf.add(exFM_out, nn_output)
        y = tf.add(y, linear_output)
        y = tf.add(y, y_bias)
...
def main(_):
    ...
    if job_name == "worker":
        ...
        tr_files = data_dir + '/train1/'+task_id_
        print("tr_files:", tr_files)
        # va_files = data_dir+"/va"
        va_files = data_dir + '/train1/04'
        print("va_files:", va_files)
        te_files = data_dir + '/train1/04'
        print("te_files:", te_files)
    else:
        tr_files = data_dir + '/train1/04'
        print("tr_files:", tr_files)
        # va_files = data_dir+"/va"
        va_files = data_dir + '/train1/04'
        print("va_files:", va_files)
